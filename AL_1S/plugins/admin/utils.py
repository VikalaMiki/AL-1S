...
TencentID = plugin_config.tencent_id
TencentKeys = plugin_config.tencent_keys
SuperUsers = global_config.superusers
CallbackNotice = plugin_config.callback_notice
...
def reply(data: str):
    ...
    try:
        data = json.loads(data)
        if data['reply'] and data['reply']['message_id']:  # todo 待优化
            return data['reply']
        else:
            return None
    except KeyError:
...
async def ban_user(gid: int, ban_list: list, time: int = None, scope: list = None):
    """
    构造禁言
    :param gid: 群号
    :param time: 时间（s)
    ...
    if 'all' in ban_list:
        ...
        for qq in ban_list:
            if int(qq) in SuperUsers or str(qq) in SuperUsers:
...
def image_moderation(img):
    try:
        ...
        client = ims_client.ImsClient(cred, 'ap-shanghai', client_profile)
...
async def check_func_status(func_name: str, gid: str) -> bool:
    ...
    if CallbackNotice:
        logger.info('错误发生在 utils.py line 342')
...
async def fi(cmd: Matcher, msg) -> None:
    await cmd.finish(msg if CallbackNotice else None)
